...
function UserCard({ user }: { user: GithubUser }) {
  return (
    <a id="github_profile_card" className={styles.card} href={user.html_url}>
      ...
      <UserInfo field="Followers" value={user.followers.toString()} />
      <UserInfo field="Following" value={user.following.toString()} />
      <UserInfo field="Public Repos" value={user.public_repos.toString()} />
    </a>
  );
}
